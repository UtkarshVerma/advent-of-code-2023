...
impl Hand {
    fn new(cards: Vec<Card>, bid: u32) -> Self {
        ...
        let max_cluster_size = clusters
            ...
            .copied()
            .unwrap_or_default();
        let joker_count = clusters.get(&Card::J).copied().unwrap_or_default();

        // J coalesces into the dominant cluster
        let cluster_count = (clusters.len() - ((joker_count > 0) as usize)).max(1);

        let hand_type = match (max_cluster_size + joker_count).min(5) {
...
QQQJA 483";
        ...
        assert_eq!(solve(input), Answer::Number(5905));
...
QQQQQ 4
AAAAA 5";
        assert_eq!(solve(input), Answer::Number(55));
    }
}
